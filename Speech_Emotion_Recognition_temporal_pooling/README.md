...
## Representaciones temporales wav2vec

El notebook `notebooks/03c_temporal_pooling_layer_mixture.ipynb` implementa statistics pooling, selección/mezcla de capas, attentive statistics pooling y Elastic Net eGeMAPS. Los experimentos reutilizan los folds y métricas existentes y trabajan exclusivamente sobre `development`.

La extracción temporal se ejecuta una sola vez activando `RUN_FEATURE_EXTRACTION`; luego los resultados se cargan desde los artefactos persistidos en `data/processed/features/` y `reports/`.
